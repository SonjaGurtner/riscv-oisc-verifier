    ...
    (define-values (m milli real-milli cpu-time)
      ...
      (if (eq? (unsat) (first m))
        ...
        (begin
          ...
          (displayln (format "Memory1 before: ~a" (evaluate test1-mem (first m))))
          (displayln (format "Memory2 before: ~a" (evaluate test2-mem (first m))))
          (displayln (format "Memory1 after Execution of ~a:\n~a" func1 (evaluate (func1 rd rs1 rs2 test1-mem) (first m))))
          (displayln (format "Memory2 after Execution of ~a:\n~a" func2 (evaluate (func2 rd rs1 rs2 test2-mem) (first m))))))))

;; (define-syntax-rule (verify_r #:func1 func1 #:func2 func2 #:space-on-stack space)
;;   (begin
;;     (display "Verifying equality of " (format "~a " func1) " and" (format "~a " func2))
;;     (define-values (m milli real-milli cpu-time)
;;       (time-apply (λ () (verifications func1 func2 space))'()))
;;     (if (eq? (unsat) (first m))
;;         (displayln (format "OK ~a" real-milli))
;;         (begin
;;           (displayln (format "FAIL ~a\n==> ~a" (first m) real-milli)) ;; we have a model => execute procedure with the values from the model
...
;;                 [v res])
;;             (displayln (format "~a = ~a" c v)))))))

(display "====Verify equality of instructions\n")
;; (verify (begin (assume (and
;;                     (< (- (bitvector->integer (read-register sp test1-mem))) (- (length (cpu-stack test1-mem)) 5))
;;                     (<= (bitvector->integer (read-register sp test1-mem)) 0)
;;                     (valid-src-reg rs1)
;;                     (valid-dest-reg rd)
;;                     (eq-mem-state test1-mem test2-mem)))
;;                (assert (eq-mem-state (addi rd rs1 imm test1-mem) (myaddi rd rs1 imm test2-mem)))))
;; (define cex (verify (begin (assume (and
;;                     (< (- (bitvector->integer (read-register sp test1-mem))) (- (length (cpu-stack test1-mem)) 6))
;;                     (<= (bitvector->integer (read-register sp test1-mem)) 0)
;;                     (valid-src-reg rs1)
;;                     (valid-src-reg rs2)
;;                     (valid-dest-reg rd)
;;                     (eq-mem-state test1-mem test2-mem)))
;;                            (assert (eq-mem-state (add rd rs1 rs2 test1-mem) (myadd rd rs1 rs2 test2-mem))))))
(verify_r
 #:func1 ror
 #:func2 myadd
 #:space-on-stack 6)
