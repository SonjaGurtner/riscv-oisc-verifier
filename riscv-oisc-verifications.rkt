...
(define (verify-func func1 func2 space r_type)
  (verify
   (begin
     (assume
      (and
   ...
   (begin
     (assume
      (and
       ...
       (bveq (bvsmod (bvneg (read-register sp test1-mem)) (int32 4)) (int32 0))
       (bvsge (read-register sp test1-mem) (int32 -10000)) ;; not really useful except for excluding (bv #x80000000 32) whose negation is negative too...
       (bvslt (read-register sp test1-mem) (int32 0)) ;; stack is always negative
       (valid-src-reg rs1)
       (valid-dest-reg rd)
       (eq-mem-state test1-mem test2-mem)))
...
(define-syntax-rule (verify-eq #:func1 func1 #:func2 func2 #:space-on-stack space #:r_type r_type)
  (begin
    ...
    (if (eq? (unsat) (first m))
        ...
        (let ([cex (complete-solution (first m) (list rs1 rs2 rd test-r1 test-r2 test-r3 test-r4 test-r5 test-r6 test-r7 test-r8 test-r9 test-r10 test-r11 test-r12 test-r13 test-r14 test-r15 test-r16 test-r17 test-r18 test-r19 test-r20 test-r21 test-r22 test-r23 test-r24 test-r25 test-r26 test-r27 test-r28 test-r29 test-r30 test-r31
  ...
  (begin
    ...
    (if (eq? (unsat) (first m))
        ...
        (let ([cex (complete-solution (first m) (list rs1 rs2 rd test-r1 test-r2 test-r3 test-r4 test-r5 test-r6 test-r7 test-r8 test-r9 test-r10 test-r11 test-r12 test-r13 test-r14 test-r15 test-r16 test-r17 test-r18 test-r19 test-r20 test-r21 test-r22 test-r23 test-r24 test-r25 test-r26 test-r27 test-r28 test-r29 test-r30 test-r31
          ...
          (displayln (format "FAIL ~a\n==> ~a ms" (first m) real-milli))
          (displayln (format "Arguments ~a" (evaluate (list rd rs1 imm) cex)))
          (displayln "Memory before Execution")
          (for ([k (range 32)] [i (evaluate (cpu-registers test1-mem) cex)])
            (displayln (format "x~a ~a" k i)))
          (let ([mem1-after (evaluate (func1 rd rs1 imm test1-mem) cex)] [mem2-after (evaluate (func2 rd rs1 imm test2-mem) cex)])
            (displayln (format "Memory (Registers) after Execution of ~a and ~a:" func1 func2))
            (for ([k (range 32)] [i (evaluate (cpu-registers mem1-after)(first m))] [j (evaluate (cpu-registers mem2-after) cex)])
              (displayln (format "x~a ~a \t2 ~a \t ~a" k i j (if (bveq i j) "" "DIFFERENT"))))
            (displayln "Memory (Stack) after Execution:")
            (for ([k (range 64)] [i (evaluate (cpu-stack mem1-after) cex)] [j (evaluate (cpu-stack mem2-after) cex)])
              (displayln (format "x~a ~a \t2 ~a \t ~a" k i j (if (bveq i j) "" "DIFFERENT")))))))))
